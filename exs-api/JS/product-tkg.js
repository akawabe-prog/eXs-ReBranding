...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // APIのベースURL設定（未設定の場合）
    if (!ApiRequester.apiBaseUrl) {
        ApiRequester.setApiBaseUrl('https://api-e.customjapan.net/api/v1');
    }
...
function renderProductTypes(types, container) {
    ...
    types.forEach(type => {
        ...
        label.innerHTML = `
            <input type="radio" name="product_type" class="peer sr-only" data-price="${type.price}" data-rakuten-url="${type.rakutenUrl}" data-yahoo-url="${type.yahooUrl}" ${checkedAttr}>
...
function setupAddToCart(btn) {
    btn.addEventListener('click', async () => {
        ...
        try {
            // カート追加API呼び出し (IDは仮で固定値またはデータ属性から取得)
            await CartApiRequester.addItemsToCart({ id: '27882603', quantity: 1, site: 'exs' });
            alert('カートに追加しました');
        } catch (e) {
